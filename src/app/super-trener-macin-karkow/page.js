...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-[#FF2A2A] selection:text-white overflow-hidden">
      <Navbar slug="super-trener-macin-karkow" companyName="super trener macin" dark={true} />
      ...
      <section className="relative w-full h-[100svh] flex flex-col justify-end pb-20 overflow-hidden px-4 md:px-12 z-10">
        ...
        >
          <motion.div variants={fadeInUp} className="mb-4 flex items-center gap-4">
            ...
            <span className="text-[#FF2A2A] font-bold tracking-[0.2em] uppercase text-sm">Nie ma wymówek.</span>
          </motion.div>
          
          <motion.h1 
            variants={fadeInUp} 
            className="text-[12vw] sm:text-[10vw] font-black uppercase leading-[0.85] tracking-tighter mb-8"
          >
            Obudź w <br/>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FF2A2A] to-[#FF5555]">Sobie Bestię.</span>
          </motion.h1>
          ...
          <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8 mt-12 border-t border-white/10 pt-8 max-w-7xl">
            <p className="text-lg md:text-xl text-gray-400 font-medium max-w-lg leading-relaxed">
              super trener macin to nie jest zwykła siłownia. To elitarny klub w Karkow, który zdefiniuje na nowo granice Twojej wytrzymałości.
            </p>
            
            <a href="/super-trener-macin-karkow/kontakt" className="group flex items-center gap-4 bg-[#FF2A2A] text-white px-10 py-5 text-sm uppercase tracking-widest font-black hover:bg-white hover:text-black transition-colors duration-300 shrink-0">
              <span>Dołącz do Elity</span>
      ...
      <section className="py-32 relative px-4 md:px-12 bg-[#050505]">
        <div className="max-w-7xl mx-auto">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeInUp} className="mb-20 flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            <div>
              <h2 className="text-5xl md:text-7xl font-black uppercase tracking-tighter mb-4 leading-none">Twój <br/><span className="text-[#FF2A2A]">Arsenał</span>.</h2>
            </div>
            <p className="text-gray-400 font-medium max-w-md text-lg">Zapewniamy bezkompromisowe warunki do treningu, oparte na nauce, technologii i czystej sile.</p>
          </motion.div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: Zap, title: "Strefa Wolnych Ciężarów", img: "https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=2070&auto=format&fit=crop" },
              { icon: Activity, title: "High Intensity Cardio", img: "https://images.unsplash.com/photo-1576678927484-cc907957088c?q=80&w=2574&auto=format&fit=crop" },
              { icon: Target, title: "Trening Funkcjonalny", img: "https://images.unsplash.com/photo-1517836357463-d25dfeac3438?q=80&w=2670&auto=format&fit=crop" }
            ].map((service, idx) => (
              <motion.div
                key={idx}
      ...
      <section className="py-0 relative overflow-hidden bg-[#050505]">
        <div className="w-full bg-[#FF2A2A] py-24 px-4 md:px-12 flex flex-col items-center text-center">
          <motion.h2 
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="text-5xl md:text-8xl font-black uppercase tracking-tighter text-black mb-8"
          >
            Czas Ucieka.<br/>Twój Ruch.
          </motion.h2>
          <motion.a 
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
            href="/super-trener-macin-karkow/kontakt" 
          ...
          >
            <span>Odbierz Pass Premium</span>
            <ChevronRight size={24} className="group-hover:translate-x-2 transition-transform" />
          </motion.a>
        </div>
      </section>

      <Footer companyName="super trener macin" />
    </div>
  );
}