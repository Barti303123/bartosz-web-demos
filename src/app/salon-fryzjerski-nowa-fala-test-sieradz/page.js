...
// Obrazy i ikony kafli — dekoracyjne; treść (tytuł/opis) pochodzi z content.services.
const BENTO_IMAGES = [
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#FBF9F6] text-[#3A332C] font-sans selection:bg-[#E3D4C4] selection:text-[#3A332C] overflow-hidden">
      <Navbar slug="salon-fryzjerski-nowa-fala-test-sieradz" companyName="Salon Fryzjerski Nowa Fala (TEST)" dark={false} />
      ...
      <section className="relative min-h-[100vh] flex flex-col lg:flex-row pt-20 lg:pt-0">
        <div className="w-full lg:w-[45%] flex flex-col justify-center px-8 lg:px-20 py-20 z-10 bg-[#FBF9F6]">
          <motion.div initial="hidden" animate="visible" variants={stagger} className="max-w-xl">
            ...
            <motion.div variants={fadeInUp}>
              <a href="/salon-fryzjerski-nowa-fala-test-sieradz/kontakt" className="group inline-flex items-center gap-6 text-sm uppercase tracking-widest font-medium">
        ...
        <div className="w-full lg:w-[55%] h-[60vh] lg:h-screen relative overflow-hidden">
          ...
          <div className="absolute top-1/2 -left-6 lg:left-8 -translate-y-1/2 -rotate-90 origin-left hidden md:block">
            <p className="text-white/40 uppercase tracking-[1em] text-xs font-bold mix-blend-overlay">Sieradz</p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <div className="max-w-2xl">
              <h2 className="text-4xl lg:text-6xl font-serif mb-6 leading-tight">Nasze <br/><span className="italic text-[#C1A88B]">zabiegi</span> i usługi.</h2>
            </div>
            <a href="/salon-fryzjerski-nowa-fala-test-sieradz/o-nas" className="text-sm uppercase tracking-widest border-b border-[#C1A88B] text-[#C1A88B] pb-1 hover:text-[#3A332C] hover:border-[#3A332C] transition-colors mt-8 md:mt-0">Odkryj Nasze Podejście</a>
          </motion.div>

          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-[400px]">
            {/* Box 1 (Large) — usługa 1 */}
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="md:col-span-8 relative rounded-3xl overflow-hidden group cursor-pointer">
              <img src={BENTO_IMAGES[0]} alt="Zabieg" className="absolute inset-0 w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"></div>
              <div className="absolute bottom-0 left-0 p-10 text-white">
                <Wind className="text-[#C1A88B] mb-4 w-8 h-8" />
                <h3 className="text-3xl font-serif mb-2">{services[0]?.title}</h3>
                <p className="text-white/70 font-light max-w-md">{services[0]?.description}</p>
              </div>
            </motion.div>

            {/* Box 2 (Small) — usługa 2 */}
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="md:col-span-4 relative rounded-3xl overflow-hidden group cursor-pointer bg-[#3A332C] text-white p-10 flex flex-col justify-end hover:bg-[#2A231C] transition-colors">
              ...
              <h3 className="text-2xl font-serif mb-2">{services[1]?.title}</h3>
              <p className="text-white/60 font-light text-sm">{services[1]?.description}</p>
            </motion.div>

            {/* Box 3 (Small) — usługa 3 */}
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="md:col-span-5 relative rounded-3xl overflow-hidden group cursor-pointer">
              <img src={BENTO_IMAGES[1]} alt="Zabieg" className="absolute inset-0 w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" />
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 text-white z-10">
                ...
                <h3 className="text-2xl font-serif">{services[2]?.title}</h3>
              </div>
            </motion.div>
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="md:col-span-7 relative rounded-3xl overflow-hidden bg-[#E3D4C4] p-10 flex flex-col justify-center items-start group">
              <p className="text-[#3A332C] font-serif text-2xl md:text-3xl leading-snug mb-6">{content.about.heading}</p>
              <p className="text-[#3A332C]/70 font-light leading-relaxed">{content.about.body}</p>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="py-32 px-4 relative overflow-hidden bg-white flex justify-center items-center">
        ...
        >
          ...
          <h2 className="text-4xl md:text-6xl font-serif mb-10 text-[#3A332C]">{content.contact.heading}</h2>
          <a href="/salon-fryzjerski-nowa-fala-test-sieradz/kontakt" className="inline-flex items-center justify-center px-12 py-5 bg-[#3A332C] text-white rounded-full uppercase tracking-widest text-sm hover:bg-[#C1A88B] transition-colors duration-500 shadow-xl shadow-black/10">
            {content.hero.cta}
          </a>
        </motion.div>
      </section>

      <Footer companyName="Salon Fryzjerski Nowa Fala (TEST)" />
    </div>
  );
}
