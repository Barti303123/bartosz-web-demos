  ...
  return (
    <main className="bg-slate-950 text-white">
      ...
      <section className="pt-32 pb-12 text-center px-6">
        ...
        <motion.h1 initial="hidden" animate="visible" variants={fade} transition={{ duration: 0.8, delay: 0.1 }} className="font-serif text-5xl md:text-6xl font-bold">{"prawnik antek"}</motion.h1>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fade} transition={{ duration: 0.7 }}>
            <h2 className="font-serif text-3xl md:text-4xl font-semibold mb-6 leading-tight">Dziedzictwo sprawiedliwości<br />w <span className="text-[#D4A853]">{"Zduńska wola"}</span></h2>
            <p className="text-slate-300 leading-relaxed mb-5">Od ponad 15 lat {"prawnik antek"} jest zaufaną nazwą w dziedzinie {"Kancelaria Prawna"} w {"Zduńska wola"}. Nasi prawnicy łączą głęboką wiedzę prawniczą z niezachwianym zaangażowaniem w sprawy każdego klienta.</p>
            <p className="text-slate-400 leading-relaxed mb-8">Od skomplikowanych sporów sądowych po delikatne negocjacje, wnosimy strategiczną jasność i nieustępliwe przygotowanie. Nasze biuro przy {"ul. Główna 1, Zduńska wola"} stanowi fundament naszej praktyki społecznej.</p>
            <a href={`/site/prawnik-antek-zdunska-wola/contact`} className="inline-block border-2 border-[#D4A853] text-[#D4A853] hover:bg-[#D4A853] hover:text-slate-950 font-semibold px-8 py-3 rounded transition-colors">Skontaktuj się z nami</a>
          </motion.div>
